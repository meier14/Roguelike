...
#mapname = None
#gameid = None
#gameid is generated for each save file and is used to identify the maps belonging to a savegame
"""Each overworld area is an array of maps
	overworld maps are called 'mapname' + 'overworldx' + 'x' 'overworldy' e.g. swamp1x3
	dungeon maps are called 'mapname' + 'dungeonlevel' e.g.  castle9"""
...
def mainmenu():
	...
	while not libtcod.console_is_window_closed():
		...
		libtcod.console_print_ex(0,SCREEN_WIDTH/2,SCREEN_HEIGHT/2-4,libtcod.BKGND_NONE,libtcod.CENTER,'TALE OF THE EXILE')
...
def newgame():
	global player,inventory,spells,gamemsgs,gamestate,gameid,areaflag,dungeonlevel
	global objects
	global mapname
	global dungeonlevel
	gameid = libtcod.random_get_int(0,0,1000)
	mapname = 'dungeon'
	areaflag = DUNGEON
	dungeonlevel = 1
	player = Object(25,23, '@',"player",libtcod.white,None,Fighter(10000,2,5,playerdeath,ai = None,magic = 30,magicpower = 5))
	gamemsgs = []
	gamestate = 'playing'
	inventory = []
	spells = []	
...
def initializefov():
	...
	for y in range(MAP_HEIGHT):
		for x in range(MAP_WIDTH):
			libtcod.map_set_properties(fov_map,x,y,not map[x][y].blocked(),not map[x][y].blocksight())
...
def savegame():
	...
	file['stairindex'] = objects.index(stairs)
	file['upstairindex'] = objects.index(upstairs)
	file['areaflag'] = areaflag
	file['mapname'] = mapname
	if areaflag == DUNGEON:
		file['dungeonlevel'] = dungeonlevel
	elif areaflag == OVERWORLD:
		file['overworldx'] = overworldx
		file['overworldy'] = overworldy
...
def loadgame():
	global map,objects,player,inventory,gamemsgs,gamestate,gameid,spells,stairs,upstairs,areaflag,dungeonlevel,overworldx,overworldy,mapname
	...
	stairs = objects[file['stairindex']]
	upstairs = objects[file['upstairindex']]
	areaflag = file['areaflag']
	if areaflag == DUNGEON:
		dungeonlevel = file['dungeonlevel']
	elif areaflag == OVERWORLD:
		overworldx = file['overworldx']
		overworldy = file['overworldy']
...
def unloadmap():
	global objects,player,stairs,upstairs
	if areaflag == DUNGEON:
		os.system('del ' + mapname + str(dungeonlevel))
		file = shelve.open(mapname + str(dungeonlevel),'n')
		file['dungeonlevel'] = None
		file['dungeonlevel'] = dungeonlevel
	elif areaflag == OVERWORLD:
		file = shelve.open(mapname + str(overworldx) + 'x' + str(overworldy))
		file['overworldx'] = None
		file['overworldy'] = None
		file['overworldx'] = overworldx
		file['overworldy'] = overworldy
	else:	raise ValueError('Invalid areaflag ' + str(areaflag))
	file['gameid'] = None
	file['map'] = None
	file['areaflag'] = None
	file['mapname'] = None
	...
	file['areaflag'] = areaflag
	file['mapname'] = mapname
	file['stairx'] = stairs.x
	file['stairy'] = stairs.y
	file['upstairx'] = upstairs.x
	file['upstairy'] = upstairs.y
...
def loadmap(newmapname):
	global objects,map,player,stairs,upstairs
	try:
		file = shelve.open(newmapname,'r')
		if file['gameid'] == gameid:
			areaflag = file['areaflag']
			if areaflag == OVERWORLD:
				overworldx = file['overworldx']
				overworldy = file['overworldy']
			elif areaflag == DUNGEON:
				dungeonlevel = file['dungeonlevel']
			else: raise ValueError('Invalid areaflag ' + str(file['areaflag']))
			map = [[Tile(WALL)
				for y in range(MAP_HEIGHT)]
					for x in range(MAP_WIDTH)]
			map = file['map']
			...
			stairs.y = file['stairy']
			upstairs.x = file['upstairx']
			upstairs.y = file['upstairy']
			file.close()
			initializefov()
		else:
			return False #in the nextlevel() function, generate new level
	except:
		return False
	#objects[file['playerindex']] = player
	#player = memplayer
	#objects[file['stairindex']] = stairs
	#objects[file['upstairindex']] = upstairs
...
def nextlevel():
	...
	global upstairs
	global stairs
	message('You take a minute to rest',libtcod.light_violet)
	player.fighter.heal(player.fighter.maxhp/2)
	unloadmap()
	dungeonlevel += 1
	fgameid = 0
	try:
		if loadmap(mapname + str(dungeonlevel)) is not False:
			player.x = upstairs.x
			player.y = upstairs.y
		else:
...
def prevlevel():
	global dungeonlevel,objects,player,stairs,upstairs
	unloadmap()
	dungeonlevel -= 1
	try:
...
class BasicMonster:
	def __init__(self):
		global fov_map
		self.pathtoplayer = False #libtcod.path_new_using_map(fov_map)
	def turnaction(self):
		monster = self.owner
		if not self.pathtoplayer or self.pathtoplayer is False:
			self.pathtoplayer = libtcod.path_new_using_map(fov_map)
		elif monster.active and self.pathtoplayer is not False:
			try:
...
def castfireball():
	...
	for obj in objects:
		...
		if obj.fighter and obj.distance(x,y) <= fireballradius:
...
def makedungeonmap():
	...
	stairs.sendtoback()
	upstairs = Object(None,None,'>','Stairs leading up',libtcod.white)
		...
		for x in range(MAP_WIDTH):
			...
			if not visible:
				...
				if type == WALL:
					...
					libtcod.console_set_char_background(con,x,y,libtcod.light_gray,libtcod.BKGND_SET)
